    ...
    provideCompletionItems(document, position, token, context) {
        ...
        if (/!\[[^\]]*?\]\([^\)]*$/.test(textBefore)) {
            ...
            return vscode_1.workspace.findFiles((dir.length == 0 ? '' : dir + '/') + '**/*.{png,jpg,jpeg,svg,gif}', '**/node_modules/**').then(uris => uris.map(uri => {
                ...
                let item = new vscode_1.CompletionItem(relPath, vscode_1.CompletionItemKind.File);
                // Add image preview
                ...
                item.documentation = new vscode_1.MarkdownString(`![${relPath}](${uri.fsPath}|width=${dimensions.width},height=${dimensions.height})`);
                return item;
            }));
        }
